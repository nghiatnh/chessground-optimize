...
function pieceCloseTo(s: State, pos: cg.NumberPair): boolean {
  const asWhite = board.whitePov(s),
    bounds = s.dom.bounds(),
    radiusSq = Math.pow(bounds.width / cg.boardSize.width, 2);
    ...
    if (!origPiece || !util.samePiece(origPiece, cur.piece)) cancel(s);
      ...
      if (cur.started) {
        ...
        util.translate(cur.element, [
          cur.pos[0] - bounds.left - bounds.width / (cg.boardSize.width * 2),
          cur.pos[1] - bounds.top - bounds.height / (cg.boardSize.height * 2),
        ]);
