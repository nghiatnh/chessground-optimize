...
function orient(pos: cg.Pos, color: cg.Color): cg.Pos {
  return color === 'white' ? pos : [cg.boardSize.width - 1 - pos[0], cg.boardSize.height - 1 - pos[1]];
}
...
function pos2user(pos: cg.Pos, bounds: ClientRect): cg.NumberPair {
  const xScale = 1;
  const yScale = 1;
  return [(pos[0] - (cg.boardSize.width - 1) / 2) * xScale, ((cg.boardSize.height - 1) / 2 - pos[1]) * yScale];
}
