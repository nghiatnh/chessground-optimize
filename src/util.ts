...
export const pos2key = (pos: cg.Pos): cg.Key => allKeys[cg.boardSize.height * pos[0] + pos[1]];

export const key2pos = (k: cg.Key): cg.Pos => [
  k.charCodeAt(0) - cg.files[0].charCodeAt(0),
  k.charCodeAt(1) - cg.ranks[0].charCodeAt(0),
];
...
export const posToTranslate =
  ...
  (pos, asWhite) =>
    [
      ((asWhite ? pos[0] : cg.boardSize.width - 1 - pos[0]) * bounds.width) / cg.boardSize.width,
      ((asWhite ? cg.boardSize.height - 1 - pos[1] : pos[1]) * bounds.height) / cg.boardSize.height,
    ];
...
export function computeSquareCenter(key: cg.Key, asWhite: boolean, bounds: ClientRect): cg.NumberPair {
  ...
  if (!asWhite) {
    pos[0] = cg.boardSize.width - 1 - pos[0];
    pos[1] = cg.boardSize.height - 1 - pos[1];
  }
  return [
    bounds.left + (bounds.width * pos[0]) / cg.boardSize.width + bounds.width / (cg.boardSize.width * 2),
    bounds.top +
      (bounds.height * (cg.boardSize.height - 1 - pos[1])) / cg.boardSize.height +
      bounds.height / (cg.boardSize.height * 2),
  ];
}
